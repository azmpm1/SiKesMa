                            ...
                            @forelse ($antrians as $index => $antrian)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="py-4 px-6 whitespace-nowrap flex items-center space-x-2">
                                        <a href="{{ route('admin.antrian.edit', $antrian->id) }}" class="px-3 py-1 bg-blue-500 text-white rounded-md text-xs hover:bg-blue-600 shadow">Edit</a>
                                        <form action="{{ route('admin.antrian.destroy', $antrian->id) }}" method="POST"
                                            onsubmit="return confirm('Anda yakin ingin menghapus data ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-3 py-1 bg-red-500 text-white rounded-md text-xs hover:bg-red-600 shadow">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
